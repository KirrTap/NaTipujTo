...
const ZapasCard: React.FC<ZapasCardProps> = ({ home_team, away_team, date }) => {
    ...
    <div style={{
      ...
      <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 8 }}>{formattedDate}</div>
          ...
          <img src={homeLogo} alt={home_team} style={{ width: 40, height: 40, objectFit: 'contain' }} />
              <input type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} style={{ width: 36, padding: 4, fontSize: 15, textAlign: 'center', borderRadius: 6, border: '1px solid #bbb' }} />
        </div>
        <span style={{ fontWeight: 600, fontSize: 18 }}>:</span>
          ...
          <button style={{ marginTop: 8, padding: '6px 16px', borderRadius: 7, background: '#1976d2', color: 'white', border: 'none', fontSize: 14, fontWeight: 600, cursor: 'pointer', minWidth: 70 }}>
            Tip
          </button>
    </div>
  );
